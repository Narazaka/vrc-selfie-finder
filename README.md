...
2. **Stage 2 (アバター識別)**: OpenCLIP (ViT-L-14) でリファレンス画像との類似度を計算し、特定アバターの写真を識別
...
YOLOv8アニメ顔検出モデル (`yolov8x6_animeface.pt`) を `models/` に配置する。
...
# 類似度閾値を変更
uv run vpm --similarity-threshold 0.65
...
| `--similarity-threshold` | `0.7` | CLIP類似度の閾値 |
...
処理結果は `.cache/` にキャッシュされる。閾値を変更して再実行する場合、顔検出やCLIP埋め込みの再計算はスキップされ数秒で完了する。
